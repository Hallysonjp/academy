...
$pagarme_settings = $this->db->get_where('settings', array('key' => 'pagarme_keys'))->row()->value;
$pagarme = json_decode($pagarme_settings);
?>
<!-- start page title -->
<div class="row ">
    <div class="col-xl-12">
...
<div class="col-md-12">
    <div class="card">
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title"><p>Configurações Pagar.me</p></h4>
                    <form class="" action="<?php echo site_url('admin/payment_settings/pagarme_settings'); ?>" method="post" enctype="multipart/form-data">
                        <div class="form-group">
                            <label><?php echo get_phrase('active'); ?></label>
                            <select class="form-control select2" data-toggle="select2" id = "pagarme_active" name="pagarme_active">
                                <option value="0" <?php if ($pagarme[0]->active == 0) echo 'selected';?>> <?php echo get_phrase('no');?></option>
                                <option value="1" <?php if ($pagarme[0]->active == 1) echo 'selected';?>> <?php echo get_phrase('yes');?></option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            <label><?php echo get_phrase('test_mode'); ?></label>
                            <select class="form-control select2" data-toggle="select2" id = "testmode" name="testmode">
                                <option value="on" <?php if ($pagarme[0]->testmode == 'on') echo 'selected';?>> <?php echo get_phrase('on');?></option>
                                <option value="off" <?php if ($pagarme[0]->testmode == 'off') echo 'selected';?>> <?php echo get_phrase('off');?></option>
                            </select>
                        </div>

                        <div class="form-group">
                            <label><?php echo get_phrase('pagarme_currency'); ?></label>
                            <select class="form-control select2" data-toggle="select2" id = "pagarme_currency" name="pagarme_currency" required>
                                <option value=""><?php echo get_phrase('select_pagarme_currency'); ?></option>
                                <?php
                                $currencies = $this->crud_model->get_stripe_supported_currencies();
                                foreach ($currencies as $currency):?>
                                    <option value="<?php echo $currency['code'];?>"
                        ...
                        <div class="form-group">
                            <label><?php echo get_phrase('test_api_key'); ?></label>
                            <input type="text" name="api_key" class="form-control" value="<?php echo $pagarme[0]->api_key;?>" required />
                        </div>

                        <div class="form-group">
                            <label><?php echo get_phrase('test_encrypted_key'); ?></label>
                            <input type="text" name="encrypted_key" class="form-control" value="<?php echo $pagarme[0]->encrypted_key;?>" required />
                        </div>
                        ...
                        <div class="row justify-content-md-center">
                            <div class="form-group col-md-6">
                                <button class="btn btn-block btn-primary" type="submit"><?php echo get_phrase('update_pagarme_keys'); ?></button>
                            </div>
                        </div>
                    </form>
                </div>
