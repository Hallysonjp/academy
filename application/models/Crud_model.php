...
class Crud_model extends CI_Model
{
    ...
    public function update_pagarme_settings()
    {
        // update pagar.me keys
        $pagarme_info = [];

        $pagarme['active']          = $this->input->post('pagarme_active');
        $pagarme['testmode']        = $this->input->post('testmode');
        $pagarme['api_key']         = $this->input->post('api_key');
        $pagarme['encrypted_key']   = $this->input->post('encrypted_key');
        $pagarme['public_live_key'] = $this->input->post('public_live_key');
        $pagarme['secret_live_key'] = $this->input->post('secret_live_key');

        array_push($pagarme_info, $pagarme);

        $data['value']    =   json_encode($pagarme_info);
        $this->db->where('key', 'pagarme_keys');
        $this->db->update('settings', $data);

        $data['value'] = html_escape($this->input->post('pagarme_currency'));
        $this->db->where('key', 'pagarme_currency');
        $this->db->update('settings', $data);
    }
